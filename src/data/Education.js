const education1 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. 3% reservation for Persons with Disabilities among Economically
        Weaker Sections/Disadvantaged Group in private schools
        <br />
        ii. Free books, uniforms and writing material to be provided by the
        schools
        <br />
        iii. Facility of a special educator
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. 40% or more disability in 21 conditions as defined by Rights of
        Persons with Disabilities Act, 2016
        <br />
        ii. Minimum age of the child for admission in nursery/pre-school is 3
        years, for KG/pre-primary is 4 years and for Class- I is 5 years
      </p>
    ),
  },
  {
    id: 3,
    title: 'What are the documents required?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Completed application form
        <br />
        ii. 2 Passport size photographs
        <br />
        iii. Self Attested photocopy of proof of date of birth. Any one of the
        following would be applicable-
        <br />
        1. Date of Birth Certificate issued by Municipal Corporation of Delhi or
        any other local body.
        <br />
        2. Anganwadi record.
        <br />
        3. Hospital/Auxiliary Nurse Midwife (ANM) register record.
        <br />
        4. An Undertaking by the Parents regarding Date of Birth
        <br />
        iv. Self Attested photocopy of residential proof. Any one of the
        following would be applicable-
        <br />
        1. Ration Card issued in the name of parents having the name of the
        child.
        <br />
        2. Domicile certificate of child or parents.
        <br />
        3. Voter-I card of any of the parents.
        <br />
        4. Electricity bill /MTNL bill/ Telephone bill /Water bill.
        <br />
        5. Bank Passbook in the name of child or parents.
        <br />
        6. Aadhar card of parents/child.
        <br />
        7. Passport in the name of any of the parents/child.
        <br />
        v. Self-attested copy of DG Category Certificate (if the admission is
        sought under DG Category (SC/ST/OBC etc.))
        <br />
        vi. Self Attested photocopy of disability certificate
      </p>
    ),
  },
  {
    id: 4,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        <a
          className='font-medium text-teal-500'
          href='http://edustud.nic.in/mis/studentadmission/webfom1.aspx'
        >
          http://edustud.nic.in/mis/studentadmission/webfom1.aspx
        </a>
      </p>
    ),
  },
  {
    id: 5,
    title: 'What are the timelines?',
    body: (
      <p className='text-sm font-light my-1'>
        The online application will be available at the beginning of the
        academic year, generally in March/April. Specific details are available
        at:
        <br />
        <a
          className='font-medium text-teal-500'
          href='http://edustud.nic.in/mis/studentadmission'
        >
          http://edustud.nic.in/mis/studentadmission
        </a>
      </p>
    ),
  },
  {
    id: 6,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Directorate of Education, Room No.12 Old
        Secretariat, near Vidhan Sabha Metro Station, Civil Lines, New Delhi,
        Delhi 110054; 011 2389 0363; diredu@nic.in
        <br />
        At Appellate Level: Delhi Commission For Protection of Child Rights, 5th
        Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone no.
        - 011-23862685; Email id - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education2 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Reader allowance at Rs. 660/- per child for 06 months to Students
        with Blindness
        <br />
        ii. Escort allowance at Rs. 1500/- per child for 06 months to Students
        with Intellectual Disabilities and Blindness having disability
        certificate
        <br />
        iii. Therapeutic Services at Rs. 2000/- per child to Students with
        Autism, Cerebral Palsy, Hearing Impairment, Intellectual Disabilities,
        Multiple Disabilities, Speech and Language Disability, Multiple
        Sclerosis, Muscular Dystrophy, Haemophilia, Thalassemia, and Dwarfism;
        and having a disability certificate;
        <br />
        iv. Transport allowance at Rs. 1200/- per child for 06 months to
        Students with Loco-motor disability, Cerebral Palsy, Multiple
        Disability, Muscular Dystrophy and v. Multiple Sclerosis; and having
        disability certificate.
        <br />
        v. Girls Stipend at Rs. 2000/- per child for 10 months to girls with
        disabilities.
        <br />
        vi. Financial assistance for large print books to children with low
        vision
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        Children with Disabilities studying Pre-School to Class XII in Schools
        of MCD, MCD Aided, NDMC, NDMC Aided, and DCB, Govt. & Govt. Aided
        Schools of Directorate of Education
      </p>
    ),
  },
  {
    id: 3,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        For guidelines for admissions for CWSN under EWS/ DG category, please
        read
        <a
          className='font-medium text-teal-500'
          href='http://edustud.nic.in/mis/studentadmission/Instructionsbi-lingual%20_new%2021012018.pdf'
        >
          http://edustud.nic.in/mis/studentadmission/Instructionsbi-lingual%20_new%2021012018.pdf
        </a>
      </p>
    ),
  },
  {
    id: 4,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Departmental Level : Directorate of Education, Room No.12 Old
        Secretariat, near Vidhan Sabha Metro Station, Civil Lines, New Delhi,
        Delhi 110054; 011 2389 0363; diredu@nic.in
        <br />
        At Appellate Level : Delhi Commission For Protection of Child Rights,
        5th Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone
        - 011-23862685; Email - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education3 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        <strong>Provisions related to Scribes/Readers:</strong>
        <br />
        i. The candidates can get their own scribe/reader (qualification one
        step below that of the candidate) or ask the examination centre
        (qualification not to be more than the minimum qualification criteria of
        the examination, however, the qualification should always be
        matriculation or above).
        <br />
        ii. The candidate opting for own scribe must fill the form provided in
        Annexure 3.
        <br />
        iii. The candidate can take different scribe/reader for writing
        different papers but only one scribe per subject.
        <br />
        <strong>Provisions related to Compensatory Time:</strong>
        <br />
        i. Compensatory time for paper of 3 hours duration: 60 minutes
        <br />
        ii. Compensatory time for paper of 2½ hours duration: 50 minutes
        <br />
        iii. Compensatory time for paper of 2 hours duration: 40 minutes
        <br />
        iv. Compensatory time for paper of 1½ hours duration: 30 minutes
        <br />
        <strong>Other provisions:</strong>
        <br />
        i. Magnifying glass/Portable video magnifier is allowed to visually
        impaired candidates
        <br />
        ii. Computer is permissible for typing answers, listening to questions
        and viewing questions in enlarged font
        <br />
        iii. Relaxation in attendance is allowed for certain categories of
        disabilities
        <br />
        iv. Registration and Examination fee for classes IX to XII will not be
        charged from visually impaired candidates
        <br />
        <strong>Specific concessions for Class 10:</strong>
        <br />
        i. Can have an option of studying one compulsory language as opposed to
        two.
        <br />
        ii. Besides one language any four from amongst the following groups can
        be offered: -Group 1: Mathematics, Science, Social Science, another
        language, Music (Any one), Painting, Home Science, Elements of Business,
        Elements of Book Keeping and Accountancy, Computer Applications -Group
        2: Any one skill subject (except automotive) for regular candidates only
        Note: Information Technology from Group 2 cannot be offered if Computer
        Applications from Group 1 is opted.
        <br />
        iii. Physio-therapeutic exercises are considered as equivalent to Health
        and Physical Education
        <br />
        iv. Separate questions will be provided for students with visual
        impairment in question with visual inputs in Social Sciences
        <br />
        <strong>Specific concessions for Class 12:</strong>
        <br />
        i. A Person With Disability, as defined under the RPWD Act 2016, has the
        option of choosing Music (Any one), Painting, Home Science as the
        subject of study.
        <br />
        ii. For practicals in the subjects of Physics, Chemistry and Biology an
        assessment containing multiple-choice questions are administered for
        PWD.
        <br />
        iii. Question papers administered in Physics, Chemistry, Mathematics and
        Biology are without any visual input iv. Alternative questions are
        provided for questions with visual inputs in History, Geography and
        Economics
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        Child must be a student of class X/XII under CBSE
      </p>
    ),
  },
  {
    id: 3,
    title: 'What are the documents required?',
    body: (
      <p className='text-sm font-light my-1'>
        1. Relevant medical certificate issued by:
        <br />
        i. Disability Certificates issued by Government hospitals controlled by
        either the Central or State Governments from the Chief Medical
        Officer/Civil Surgeon /Medical Superintendent.
        <br />
        ii. Disability Certificates issued by Recognized institutes of national
        level viz National Association for the Blind, Spastic Society of India
        etc; and
        <br />
        iii. Disability Certificates issued by Nongovernmental
        Organizations/practitioners registered with Rehabilitation Council of
        India/Central Government/State Government of the Respective State.
        <br />
        iv. The disability certificate issued by the competent authority at any
        place shall be accepted.
        <br />
        v. The proforma of the certificate regarding physical limitation for an
        examinee to write is annexed at Annexure-A
        <br />
        2. Recommendations from the Head of the School. Format for the same
        attached at Annexure B
      </p>
    ),
  },
  {
    id: 4,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Students studying in CBSE schools and desirous of availing the
        exemptions/concessions should approach the concerned Regional Office of
        CBSE through the Head of their Institution supported by relevant medical
        certificates and recommendations from the Head of the School.
        <br />
        Only those students would be considered for grant of
        exemption/concession in whose respect the relevant disability category
        has been entered during registration in Class IX and/or XI.
      </p>
    ),
  },
  {
    id: 5,
    title: 'What are the timelines?',
    body: (
      <p className='text-sm font-light my-1'>
        1. Schools will inform to the PWD candidates about
        exemptions/concessions extended by CBSE during annual examinations;
        Schools will make a list of PWD students after submitting LOC: At the
        beginning of the academic session.
        <br />
        2. Students/Parents will make the request to their School for seeking
        exemptions/concessions during annual examinations alongwith required
        Certificates/Documents: Upto 7th July of the academic year
        <br />
        3. Schools will compile all such requests in tabular form with required
        Certificates/Documents. Only one request will be sent by each school to
        the Regional Office for necessary approvals: 07 days from the date of
        activity of S.No.2 i.e. by 15th July
        <br />
        4. Regional Office will communicate deficiency, if any, to the schools:
        21days from the date of activity of S.No.3 i.e. by 05th August
        <br />
        5. Schools will communicate fulfilment of deficiency to Regional Office:
        10 days from the date of activity of S.No.4 i.e. by 15th August
        <br />
        6. Regional Office will communicate approval to the schools: 10 days
        from the date of activity of S.No.5 i.e. by 25th August
        <br />
        7. School will communicate approval of the CBSE to the students: 05 days
        from the date of activity of S.No.6 i.e. by 30th August
      </p>
    ),
  },
  {
    id: 6,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Central Board of Secondary Education, PS-1-2,
        Institutional Area I.P.Extn. Patparganj, Delhi-110 092; Phone -
        91-11-22239177-80,222359-48/04; Email - rodelhi.cbse@nic.in
        <br />
        At Appellate Level: Delhi Commission For Protection of Child Rights, 5th
        Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone -
        011-23862685; Email - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education4 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        Pre-Matric Scholarship is to support students with disabilities for
        their study in classes IX and X onwards so that the incidence of
        drop-out, especially in the transition from the elementary to the
        secondary stage is minimized.
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Full-time student of class IX or class X in a government recognised
        school.
        <br />
        ii. 40% or more disability
        <br />
        iii. Family income not more than Rs 2,50,000 per annum.
      </p>
    ),
  },
  {
    id: 3,
    title: 'What are the documents required?',
    body: (
      <p className='text-sm font-light my-1'>
        1. Photo of the applicant
        <br />
        2. Certificate of previous academic mark sheet
        <br />
        3. Fee receipt of current course year
        <br />
        4. Age proof certificate
        <br />
        5. Income certificate of the family
        <br />
        6. Bank account details
        <br />
        7. Disability certificate
      </p>
    ),
  },
  {
    id: 4,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Apply online at -
        <a
          className='font-medium text-teal-500'
          href='https://scholarships.gov.in/'
        >
          https://scholarships.gov.in/
        </a>
      </p>
    ),
  },
  {
    id: 5,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Ministry of Social Justice and Empowerment, Govt of
        India, Room no. 202, 2nd floor C-wing, Shastri Bhawan, New Delhi, Delhi
        110001; Phone - 011 2338 1001, Email- pk.singh72@nic.in
        <br />
        At Appellate Level: Delhi Commission For Protection of Child Rights, 5th
        Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone -
        011-23862685; Email - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education5 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        Post-Matric Scholarship is to support students with disabilities for
        their study in classes XI and XII onwards so that the incidence of
        drop-out, especially in the transition from the secondary to the higher
        secondary stage is minimized.
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Students studying in all recognised post-matriculation courses.
        Courses at the level of undergraduate, postgraduate degree/diploma/
        certificate are also covered under this scheme
        <br />
        ii. 40% or more disability
        <br />
        iii. Family income not more than Rs 2,50,000 per annum.
      </p>
    ),
  },
  {
    id: 3,
    title: 'What are the documents required?',
    body: (
      <p className='text-sm font-light my-1'>
        1. Photograph of the applicant.
        <br />
        2. Birth Certificate or any valid proof of age.
        <br />
        3. Disability Certificate.
        <br />
        4. Family Income Certificate.
        <br />
        5. Fee receipt.
        <br />
        6. Mark sheet of the previous final examination.
        <br />
        7. Bank account details
      </p>
    ),
  },
  {
    id: 4,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Apply online at -
        <a
          className='font-medium text-teal-500'
          href='https://scholarships.gov.in/'
        >
          https://scholarships.gov.in/
        </a>
      </p>
    ),
  },
  {
    id: 5,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Ministry of Social Justice and Empowerment, Govt of
        India, Room no. 202, 2nd floor C-wing, Shastri Bhawan, New Delhi, Delhi
        110001; Phone - 011 2338 1001, Email- pk.singh72@nic.in
        <br />
        At Appellate Level: Delhi Commission For Protection of Child Rights, 5th
        Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone -
        011-23862685; Email - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education6 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        The Scholarship for Top Class Education aims at recognizing and
        promoting quality education amongst students with disabilities by
        providing full financial support. It covers students with disabilities
        (SwDs) for pursuing studies at the level of graduate and post-graduate
        degree or diploma in notified institutes (List attached as Annexure 5)
        of excellence in education identified by DEPwD.
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Be pursuing studies at graduate/postgraduate degree/diploma level in
        notified institutions of excellence in education
        <br />
        ii. 40% or more disability
        <br />
        iii. Annual family income of less than Rs 6,00,000 per annum.
      </p>
    ),
  },

  {
    id: 3,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Apply online at -
        <a
          className='font-medium text-teal-500'
          href='https://scholarships.gov.in/'
        >
          https://scholarships.gov.in/
        </a>
      </p>
    ),
  },
  {
    id: 4,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Ministry of Social Justice and Empowerment, Govt of
        India, Room no. 202, 2nd floor C-wing, Shastri Bhawan, New Delhi, Delhi
        110001; Phone - 011 2338 1001, Email- pk.singh72@nic.in
        <br />
        At Appellate Level: Delhi Commission For Protection of Child Rights, 5th
        Floor, ISBT Building, Kashmere Gate, New Delhi, Delhi 110006; Phone -
        011-23862685; Email - dcpcr@hotmail.com
      </p>
    ),
  },
];

const education7 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        The National Fellowship for PWD aims to provide financial assistance to
        the students with disabilities for pursuing higher education leading to
        degrees such as M.Phil and Ph.D in India.
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Any student with 40% or more disability
        <br />
        ii. Admitted to full-time M.Phil./Ph.D. degree in a recognized
        university and programme
      </p>
    ),
  },
  {
    id: 3,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Apply online at -
        <a
          className='font-medium text-teal-500'
          href='https://nfpwd.ugc.ac.in/'
        >
          {' '}
          https://nfpwd.ugc.ac.in/{' '}
        </a>
      </p>
    ),
  },
  {
    id: 4,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: University Grants Commission Email -
        sosa3ugc@gmail.com
        <br />
        At Appellate Level: Office of the Chief Commissioner for Persons with
        Disabilities, Sarojini House, 6, Bhagwan Das Rd, Bhagwan Das Lane, Mandi
        House, New Delhi, Delhi 110001, Phone- 011 2338 6054, Email:
        comdis.delhi@nic.in
      </p>
    ),
  },
];

const education8 = [
  {
    id: 1,
    title: 'What are the entitlements?',
    body: (
      <p className='text-sm font-light my-1'>
        The National Overseas Scholarship for PWD aims to provides financial
        assistance to the students with disabilities who are selected for
        pursuing Master's level courses and Ph.D abroad in the following
        specified fields of study: (a) Engineering & Management (b) Pure
        Sciences and Applied Sciences (c) Agricultural Science & Medicine (d)
        Commerce, Accounting and Finance and (e) Humanities, Social Science
        including Law & Fine Arts.
      </p>
    ),
  },
  {
    id: 2,
    title: 'What are the eligibility requirements?',
    body: (
      <p className='text-sm font-light my-1'>
        i. Students with 40 % or more disability; Below 35 years and family
        income not more than Rs 6,00,000 per annum.
        <br />
        ii. For Ph.D - First class or 55% marks or equivalent grade in relevant
        Master's Degree, Experience preferred
        <br />
        iii. For Masters' Degree: - 55% marks or equivalent grade in relevant
        Bachelor's Degree, Experience preferred
      </p>
    ),
  },
  {
    id: 3,
    title: 'What is the application process?',
    body: (
      <p className='text-sm font-light my-1'>
        Offline Application is available in the link below.
        <a
          className='font-medium text-teal-500'
          href='http://health.delhigovt.nic.in/wps/wcm/connect/083fff004d3a3c1181a5f5d1e963ea50/DAJSAl.pdf?MOD=AJPERES&lmod=2016904728&CACHEID=083fff004d3a3c1181a5f5d1e963ea50'
        >
          http://health.delhigovt.nic.in/wps/wcm/connect/083fff004d3a3c1181a5f5d1e963ea50/DAJSAl.pdf?MOD=AJPERES&lmod=2016904728&CACHEID=083fff004d3a3c1181a5f5d1e963ea50
        </a>
        <br />
        Application needs to be submitted at: O/o Patient Welfare Cell, Room no
        1, 6th Floor, Directorate of Health Services, F-17, Karkardooma,
        Delhi-110032. Phone no. - 011-22306851 OR, In the O/o Medical
        Superintendent/ Medical Director of the concerned Government Hospital
      </p>
    ),
  },
  {
    id: 4,
    title: ' What are the grievance redressal bodies?',
    body: (
      <p className='text-sm font-light my-1'>
        At Department Level: Under Secretary (Scholarships), Department of
        Empowerment of Persons with Disabilities, Room No. 516, 5th Floor,
        Paryavaran Bhawan, CGO Complex, Lodhi Road, New Delhi - 110003; Email:
        pwdcgo@gmail.com
        <br />
        At Appellate Level: Office of the Chief Commissioner for Persons with
        Disabilities, Sarojini House, 6, Bhagwan Das Rd, Bhagwan Das Lane, Mandi
        House, New Delhi, Delhi 110001,011 2338 6054, Email: comdis.delhi@nic.in
      </p>
    ),
  },
];

export {
  education1,
  education2,
  education3,
  education4,
  education5,
  education6,
  education7,
  education8,
};
